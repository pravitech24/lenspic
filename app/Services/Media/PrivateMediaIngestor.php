...
class PrivateMediaIngestor {
 ...
 public function __construct(private ProtectedMediaStorage $storage,private ImageOptimizationService $optimizer){}
 public function ingest(UploadedFile $file,Group $group,User $uploader): Photo {
  ...
  try {
   ...
   $info=@getimagesize($file->getRealPath());$owner=User::findOrFail($group->creator_id);
   return DB::transaction(function()use($file,$group,$uploader,$owner,$disk,$mime,$originalKey,$optimizedKey,$thumbnailKey,$thumbnailMime,$thumbWidth,$thumbHeight,$info){
    ...
    $asset=MediaAsset::create(['uuid'=>(string)Str::uuid(),'photo_id'=>$photo->id,'group_id'=>$group->id,'owner_id'=>$owner->id,'uploader_id'=>$uploader->id,'storage_disk'=>$disk,'original_object_key'=>$originalKey,'original_filename'=>basename($file->getClientOriginalName()),'mime_type'=>$mime,'size_bytes'=>$this->storage->size($originalKey),'checksum_sha256'=>$this->storage->checksum($originalKey),'width'=>$info[0]??null,'height'=>$info[1]??null,'state'=>'ready','migration_state'=>'native']);
    foreach([['original',$originalKey,$mime,$info[0]??null,$info[1]??null],['optimized',$optimizedKey,$mime,$info[0]??null,$info[1]??null],['thumbnail',$thumbnailKey,$thumbnailMime,$thumbWidth,$thumbHeight]] as [$type,$key,$variantMime,$width,$height])MediaVariant::create(['media_asset_id'=>$asset->id,'variant_type'=>$type,'storage_disk'=>$disk,'object_key'=>$key,'mime_type'=>$variantMime,'size_bytes'=>$this->storage->size($key),'checksum_sha256'=>$this->storage->checksum($key),'width'=>$width,'height'=>$height,'state'=>'ready']);
    StorageLedgerEntry::create(['owner_id'=>$owner->id,'group_id'=>$group->id,'media_asset_id'=>$asset->id,'event_type'=>'media_ingested','byte_delta'=>$asset->size_bytes,'idempotency_key'=>'media-ingested-'.$asset->uuid]);$owner->increment('storage_used',$asset->size_bytes);return $photo;
   });
