...
class LaravelProtectedMediaStorage implements ProtectedMediaStorage {
 ...
 private function safeKey(string $key): string{if(strlen($key)>self::MAX_OBJECT_KEY_LENGTH||$key===''||str_starts_with($key,'/')||str_contains($key,'..')||str_contains($key,"\0")||str_contains($key,'\\'))throw new RuntimeException('Unsafe media key.');return $key;}
}
