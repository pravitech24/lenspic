...
class UnifiedThemeTest extends TestCase
{
    ...
    public function test_gallery_grids_have_no_rounded_utility_on_immediate_media():void{foreach([resource_path('js/Pages/Groups/Show.vue'),resource_path('js/Pages/MyPhotos.vue'),resource_path('js/Pages/Groups/Reviews.vue'),resource_path('js/Pages/Groups/Favorites.vue')]as$file){$source=file_get_contents($file);$this->assertStringContainsString('photo-grid',$source);$this->assertDoesNotMatchRegularExpression('/photo-(?:tile|grid)[^>]*rounded-|<img[^>]*rounded-/',$source);}$css=file_get_contents(resource_path('css/app.css'));$this->assertStringContainsString('border-radius:0!important',$css);}
